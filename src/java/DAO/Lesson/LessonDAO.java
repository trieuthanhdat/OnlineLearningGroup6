...
public class LessonDAO implements Serializable {
    ...
    public ArrayList<LessonDTO> getLessonQuizbySubjectID(int subjectID) throws NamingException, SQLException {
        Connection con = null;
    ...
    public int getSubjectIDbyLessonID(int lessonid) throws NamingException, SQLException {
        Connection con = null;
        PreparedStatement stm = null;
        ResultSet rs = null;
        try {
    ...
    public boolean checkIfQuizIsInUse(int quizID)
            throws NamingException, SQLException {
        for (LessonDTO lesson : lessonsList) {
            if (lesson.isStatus()) {
                if (getLessonDetailsByID(lesson.getLessonID()).getQuizID() == quizID) {
                    return true;
                }
            }
        }
    ...
    public List<LessonDTO> getLearningLessonsFromSubject(int subID) {
        List<LessonDTO> resultList = new ArrayList<>();
        for (LessonDTO lesson : lessonsList) {
            if (lesson.getSubjectID() == subID && lesson.isStatus()) {
                if (lesson.getType().toLowerCase().equals("lesson")) {
    ...
    public int getNumOfQuizzesFromSubject(int subID) {
        List<LessonDTO> resultList = new ArrayList<>();
        for (LessonDTO lesson : lessonsList) {
            if (lesson.getSubjectID() == subID && lesson.isStatus()) {
    ...
    public boolean checkSubjectHasValidLessons(int subjectID) {
        ...
        for (int i = 0; i < currLessons.size(); i++) {
            if (currLessons.get(i).getTopicID() == 0) {
                // end of current lessons is a topic
                if (i == currLessons.size()) {
                    return false;
                } else if (currLessons.get(i + 1).getTopicID() == 0) { //after a topic is a topic
                    return false;
                }
            }
        }
    ...
    public boolean addNewLesson(LessonDTO newLesson)
        ...
        try {
            con = DBHelpers.makeConnection();
            if (con != null) {
                String sql = "INSERT INTO Lesson (SubjectID, Name, "
                        + "LessonOrder, Type, TopicID, Status) "
                        + "VALUES (?, ?, ?, ?, ?, ?)";
                stm = con.prepareStatement(sql);                
                stm.setInt(1, subID);
                stm.setNString(2, newLesson.getName());
                stm.setInt(3, order);
                stm.setString(4, newLesson.getType());
                stm.setInt(5, topicID);
                stm.setBoolean(6, newLesson.isStatus());
