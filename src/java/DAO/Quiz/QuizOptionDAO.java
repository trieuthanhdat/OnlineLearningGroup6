...
public class QuizOptionDAO implements Serializable {
    ...
    public boolean editOptionByQuestionNo(int questionNo, String content, boolean isCorrect) throws NamingException, SQLException, ClassNotFoundException {
        boolean result = false;
        try {
            con = DBHelpers.makeConnection();

            if (con != null) {
                String sql = "UPDATE QuizQuestionOption SET Content=?, IsCorrect=?"
                        + " WHERE QuestionNo = ? ";
                stm = con.prepareStatement(sql);
                stm.setString(1, content);
                stm.setBoolean(2, isCorrect);
                stm.setInt(3, questionNo);
